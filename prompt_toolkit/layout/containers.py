...
class Window(Container):
    ...
    :param get_line_prefix: None or a callable that returns formatted text to
        be inserted before a line. It takes the window width (int), a line
        number (int), ypos (int) and a boolean indicating whether or not this
        was a soft wrap and returns formatted text. This can be used for
    ...
    def _copy_body(self, ui_content, new_screen, write_position, move_x,
        ...
        :param get_line_prefix: None or a callable that takes the window width
            (int), a line number (int), ypos (int) and a boolean indicating
            whether or not this was a soft wrap and returns formatted text.
        """
        ...
        def copy_line(line, lineno, x, y, is_input=False):
            ...
            if is_input and get_line_prefix:
                prompt = to_formatted_text(
                    get_line_prefix(width, lineno, is_soft_wrap=False))
                x, y = copy_line(prompt, lineno, x, y, False)

            if horizontal_scroll and is_input:
                skipped = horizontal_scroll
                line = explode_text_fragments(line)[horizontal_scroll:]  # XXX: support double width characters.
            else:
                skipped = 0
#            x = -horizontal_scroll  # TODO: scroll horiontally.
    ...
    def _scroll_when_linewrapping(self, ui_content, width, height):
        ...
        if line_height > height - scroll_offsets_top:
            ...
            # cursor included as well.
                    ...
                    # TODO: This should include the line prefixes off ALL lines PLUS include those for the cursor position as well.
                    # TODO: When this is done, we need to remove all the ypos from the get_line_prefix calls
                    # TODO: test with '' vs None as line_continuation in prompt.
                    # TODO: and clean up.
            line = (explode_text_fragments(to_formatted_text(self.get_line_prefix(
                width, ui_content.cursor_position.y, False)))
                                           +
                explode_text_fragments(ui_content.get_line(ui_content.cursor_position.y)))
            prefix_len = len(explode_text_fragments(to_formatted_text(self.get_line_prefix(
                width, ui_content.cursor_position.y, False))))

            text_before_cursor = fragment_list_to_text(line[:ui_content.cursor_position.x + 1 + prefix_len])   # TODO: this is wrong.
    ...
    def _scroll_without_linewrapping(self, ui_content, width, height):
        ...
        if self.get_line_prefix:
            current_line_prefix_width = fragment_list_width(to_formatted_text(
                self.get_line_prefix(width, ui_content.cursor_position.y, False)))
        else:
